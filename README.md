...
- **One name per room:** a room holds each name once, across both parties, compared without regard to case. Joining as `QA-BOT` while `qa-bot` is present or still a member (for example, timed out), or as AI `X` while human `x` is, gets `409 handle_taken`. The exact same name as the other party gets `403 invalid_party`. Re-joining with your own name in the same party works as before, and the name keeps the case it was first joined with.
